...
//SIMPLE방법
//JAVA MAP -> {ename=smith, empno=7733, sal=600}
//JSON -> {"ename":"smith", "empno":"7733", "sal":"600"}
...
public class JsonTest {
	...
	public static void main(String[] args) {
		...
		try {
			map2 = (HashMap<String, String>)parser.parse(jsonStr); //맵으로 바꿨으니 꺼내자 -> 맵으로 파싱한 것이다. -> 오브젝트로 떤진다
		...
		//자바유틸에있는 자바순수기능이다. SIMPLE JSON 쓰는 김에 object활용해보자
		JSONObject obj = new JSONObject();
		try {
			obj = (JSONObject)parser.parse(jsonStr); //객체로바꿔서 obj에게 전달
			
			//String mapToStr = obj.toJSONString(map2); //맵을 넣어서 스트링을 바로 받을수도있다. 위 map2 객체는 해쉬맵이니 글씨로 받나 확인하자
			
			//바로 윗줄은 static이라 클래스이름.() 으로 메모리에 올렸잖아. obj가 아닌 클래스이름 넣어주는게 맞지.
		    ...
		    mapToStr = obj.toString(); //map을 이용해서 toString으로 하면 글자로 바뀐다. 
		    System.out.println(mapToStr);
