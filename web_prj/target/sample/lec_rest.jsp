...
참고 : https://korean.visitseoul.net/restaurants
...
<script>
	...
	id값인 1__str_str_btn이 눌렸을때, 어떻게 가냐?
	data : 인 쿼리스트링 으로 접근하면된다.
	이게귀찮으면 #1 사용하자
	
	콜백방식으로 결과가 오면 찍자.
	리스트나 객체나 맵의 형태라면 하나하나 루프돌려서 하나씩 뺴야한다.
	...
	*/
	$("#1__Str_Str_Btn").click(function() {
	    ...
	    $.ajax({
	        ...
	        success: function(myval) {
	            ...
	            var myval_obj = JSON.parse(myval);   //obj = JSON.parse(str); -> JSON모양의 글자를 객체 JSON으로 변환 -> 변환해야됨!
	...
	$("#2__JsonStr_Str_Btn").click(  function(){
		...
		//제이슨 모양의 스트링을 보내는 것이다.
		$.ajax({
			method      : "POST",
	        url         : "<%=request.getContextPath()%>/RestServlet?pagecode=R002",
	        contentType : "application/x-www-form-urlencoded; charset=UTF-8",
	        data 		: "MY_JSONKEY=" + JSON.stringify(objData),    //이건 application/x-www-form-urlencoded; charset=UTF-8 형태이다. -> 이부분이 핵심이다. 내가보낸 오브젝데이터는 이 줄이다. var objData 글자형식({"name":"kim")} 형식으론 못보내니 스트링기파이로 감싸서 보냈다.
		...
		$.ajax({
	        ...
	        dataType    : "json",    //생략가능
	  		...
	  		success     : function(myval){ //{"status":"200", "message":"R004 응답 ok"}
	  									  ...
	  									  console.log(myval['status'] + "," + myval['message']);   //파싱없이받았음
	  									 }
		});
	});
	...
	//제이슨으로 보내고 제이슨으로 받겠다 -> 이대로 못나가니 일단 스트링기파이로 감싸서 나가긴할꺼야
