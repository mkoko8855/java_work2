							...
							<nav class="sb-sidenav-menu-nested nav accordion"
								...
								<div class="collapse" id="pagesCollapseAuth"
									...
									<nav class="sb-sidenav-menu-nested nav">
										...
										<!-- ---------------  로그인/로그아웃 처리 ------------------------- -->
										<c:set var="userid value=" ${sessionScope.KEY_SESS_USERID}"/>
										<c:set var="uname value=" ${sessionScope.KEY_SESS_UNAME}"/>
										<c:set var="grade value=" ${sessionScope.KEY_SESS_GRADE}"/>


										<c:choose>
											<c:when test="${not empty grade}">
												<a class="nav-link"
													href="${pageContext.request.contextPath}/UserServlet">Logout</a>
											</c:when>
							...
							<tbody>
								<c:forEach var="bvo" items="${requestScope.KEY_BOARDLIST}">
									<tr>
										...
										<td>
										<a href="${pageContext.request.contextPath}/BoardServlet?pagecode=B002&seq=${bvo.seq}">
										${bvo.title}
										</a>
									</td>
										<td>${bvo.regid}</td>
